...
ARTIFACT_DIR="$ROOT_DIR/.artifacts/phase11-e2e"
WEB_PORT=5174
SERVER_PORT=5081
WEB_PID=""
SERVER_PID=""
...
env Server__Port="$SERVER_PORT" Server__SnapshotRate=5 Server__AllowedWebSocketOrigins="http://127.0.0.1:$WEB_PORT" Simulation__TickRate=30 Simulation__SpatialCellSize=64 Simulation__InitialAgentCount=0 Simulation__SavePath="$ROOT_DIR/tests/fixtures/phase11-road-network.save.json" dotnet run --project "$ROOT_DIR/src/gateway/MachiVerseWorks.Server.csproj" --configuration Release --no-build >"$ARTIFACT_DIR/server.log" 2>&1 & SERVER_PID=$!
wait_http "http://127.0.0.1:$SERVER_PORT/health"
BROWSER_URL="http://127.0.0.1:$WEB_PORT/tests/browser/e2e.html?agents=0&mode=road&server=ws%3A%2F%2F127.0.0.1%3A$SERVER_PORT%2Fws"
...
if health.get("roadSegments") != 5:
    raise SystemExit(f"Expected 5 RoadSegments, got {health.get('roadSegments')}")
print(json.dumps(health, ensure_ascii=False, indent=2))
PY

echo "Phase 11 Save -> Server -> Browser E2E passed."
