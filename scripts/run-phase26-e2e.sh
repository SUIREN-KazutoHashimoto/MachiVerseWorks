...
ARTIFACT_DIR="$ROOT_DIR/.artifacts/phase26-e2e"
WEB_PORT=5183
SERVER_PORT=5090
WEB_PID=""
SERVER_PID=""
mkdir -p "$ARTIFACT_DIR"; rm -f "$ARTIFACT_DIR"/*
cleanup() { if [[ -n "$SERVER_PID" ]] && kill -0 "$SERVER_PID" 2>/dev/null; then kill "$SERVER_PID" 2>/dev/null || true; wait "$SERVER_PID" 2>/dev/null || true; fi; if [[ -n "$WEB_PID" ]] && kill -0 "$WEB_PID" 2>/dev/null; then kill "$WEB_PID" 2>/dev/null || true; wait "$WEB_PID" 2>/dev/null || true; fi; }
...
echo "Phase 26 Optical Server -> Browser E2E passed."
