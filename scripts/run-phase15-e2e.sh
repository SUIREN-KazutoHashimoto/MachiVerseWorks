...
ARTIFACT_DIR="$ROOT_DIR/.artifacts/phase15-e2e"
WEB_PORT=5175
SERVER_PORT=5083
WEB_PID=""
SERVER_PID=""
...
wait_http "http://127.0.0.1:$WEB_PORT/tests/browser/population-e2e.html"

env Server__Port="$SERVER_PORT" Server__SnapshotRate=10 Server__AllowedWebSocketOrigins="http://127.0.0.1:$WEB_PORT" Simulation__TickRate=30 Simulation__SpatialCellSize=64 Simulation__InitialAgentCount=0 Simulation__PopulationFixture=true dotnet run --project "$ROOT_DIR/src/gateway/MachiVerseWorks.Server.csproj" --configuration Release --no-build >"$ARTIFACT_DIR/server.log" 2>&1 & SERVER_PID=$!
wait_http "http://127.0.0.1:$SERVER_PORT/health"
BROWSER_URL="http://127.0.0.1:$WEB_PORT/tests/browser/population-e2e.html?server=ws%3A%2F%2F127.0.0.1%3A$SERVER_PORT%2Fws"
...
if health.get("households") != 1 or health.get("persons") != 1:
    raise SystemExit(f"Population fixture health mismatch: {health}")
print(json.dumps(health, ensure_ascii=False, indent=2))
PY

echo "Phase 15 Population Server -> Browser E2E passed."
