...
ARTIFACT_DIR="$ROOT_DIR/.artifacts/phase19-e2e"
WEB_PORT=5176
SERVER_PORT=5083
WEB_PID=""
SERVER_PID=""
...
wait_http "http://127.0.0.1:$WEB_PORT/tests/browser/multimodal-transit-e2e.html"

# Keep the finite deterministic transit fixture observable long enough for slower CI browser startup.
env Server__Port="$SERVER_PORT" Server__SnapshotRate=10 Server__AllowedWebSocketOrigins="http://127.0.0.1:$WEB_PORT" Simulation__TickRate=5 Simulation__InitialAgentCount=0 Simulation__MultimodalTransitFixture=true dotnet run --project "$ROOT_DIR/src/MachiVerseWorks.Server/MachiVerseWorks.Server.csproj" --configuration Release --no-build >"$ARTIFACT_DIR/server.log" 2>&1 & SERVER_PID=$!
wait_http "http://127.0.0.1:$SERVER_PORT/health"
BROWSER_URL="http://127.0.0.1:$WEB_PORT/tests/browser/multimodal-transit-e2e.html?server=ws%3A%2F%2F127.0.0.1%3A$SERVER_PORT%2Fws"
...
echo "Phase 19 Server -> Browser multimodal transit E2E passed."
