...
ARTIFACT_DIR="$ROOT_DIR/.artifacts/phase17-e2e"
WEB_PORT=5174
SERVER_PORT=5081
WEB_PID=""
SERVER_PID=""
...
wait_http "http://127.0.0.1:$WEB_PORT/tests/browser/railway-e2e.html"

env Server__Port="$SERVER_PORT" Server__SnapshotRate=5 Server__AllowedWebSocketOrigins="http://127.0.0.1:$WEB_PORT" Simulation__InitialAgentCount=0 Simulation__SavePath="$ROOT_DIR/tests/fixtures/phase17-railway.save.json" dotnet run --project "$ROOT_DIR/src/server/MachiVerseWorks.Server.csproj" --configuration Release --no-build >"$ARTIFACT_DIR/server.log" 2>&1 & SERVER_PID=$!
wait_http "http://127.0.0.1:$SERVER_PORT/health"
BROWSER_URL="http://127.0.0.1:$WEB_PORT/tests/browser/railway-e2e.html?server=ws%3A%2F%2F127.0.0.1%3A$SERVER_PORT%2Fws"
...
if health.get("trackSegments") != 5:
    raise SystemExit(f"Expected 5 TrackSegments, got {health.get('trackSegments')}")
print(json.dumps(health, ensure_ascii=False, indent=2))
PY

echo "Phase 17 Save -> Server -> Browser railway E2E passed."
