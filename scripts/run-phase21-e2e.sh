...
ARTIFACT_DIR="$ROOT_DIR/.artifacts/phase21-e2e"
WEB_PORT=5178
SERVER_PORT=5085
WEB_PID=""
SERVER_PID=""
...
wait_http "http://127.0.0.1:$WEB_PORT/tests/browser/phase21-e2e.html"

env Server__Port="$SERVER_PORT" Server__SnapshotRate=10 Server__AllowedWebSocketOrigins="http://127.0.0.1:$WEB_PORT" Simulation__InitialAgentCount=0 Simulation__EconomyFixture=true dotnet run --project "$ROOT_DIR/src/server/MachiVerseWorks.Server.csproj" --configuration Release --no-build >"$ARTIFACT_DIR/server.log" 2>&1 & SERVER_PID=$!
wait_http "http://127.0.0.1:$SERVER_PORT/health"
BROWSER_URL="http://127.0.0.1:$WEB_PORT/tests/browser/phase21-e2e.html?server=ws%3A%2F%2F127.0.0.1%3A$SERVER_PORT%2Fws"
...
echo "Phase 21 economy Server -> Browser E2E passed."
