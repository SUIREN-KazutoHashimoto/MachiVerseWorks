...
public static class OpticalProtocolCodec
{
    ...
    private static bool IsValid(OpticalSnapshotMessage m)
    {
        ...
        foreach (var v in m.Demands) if (v.DemandId == 0 || v.NodeId == 0 || !Enum.IsDefined(v.Kind) || !Enum.IsDefined(v.QualityState) || !Positive(v.BaseDemandGigabitsPerSecond) || !NonNegative(v.DemandGigabitsPerSecond) || !NonNegative(v.AllocatedGigabitsPerSecond) || !NonNegative(v.EstimatedLatencyMilliseconds)) return false;
        return true;
    }
    ...
    private static void WriteDemand(Span<byte> p, ProtocolOpticalDemand v) { WriteUInt64(p,v.DemandId); WriteUInt64(p[8..],v.NodeId); p[16]=(byte)v.Kind; WriteUInt64(p[17..],v.BuildingId); WriteUInt64(p[25..],v.EstablishmentId); WriteDouble(p[33..],v.BaseDemandGigabitsPerSecond); WriteDouble(p[41..],v.DemandGigabitsPerSecond); WriteDouble(p[49..],v.AllocatedGigabitsPerSecond); p[57]=(byte)v.QualityState; WriteUInt64(p[58..],v.BackhaulId); WriteDouble(p[66..],v.EstimatedLatencyMilliseconds); }
    private static ProtocolOpticalDemand ReadDemand(ReadOnlySpan<byte> p)=>new(ReadUInt64(p),ReadUInt64(p[8..]),(ProtocolOpticalDemandKind)p[16],ReadUInt64(p[17..]),ReadUInt64(p[25..]),ReadDouble(p[33..]),ReadDouble(p[41..]),ReadDouble(p[49..]),(ProtocolOpticalQualityState)p[57],ReadUInt64(p[58..]),ReadDouble(p[66..]));
