...
public static class RadioProtocolCodec
{
    ...
    private const int RadioFixedLength = 58;
    ...
    public static byte[] Serialize(RadioSnapshotMessage message, ProtocolVersion version)
    {
        ...
        var payloadLength = checked(RadioFixedLength + message.Sites.Count * SiteLength + message.Links.Count * LinkLength + message.ServiceAreas.Count * ServiceAreaLength);
        ...
        WriteRadioStatistics(p, message.Statistics);
        WriteUInt16(p[52..], checked((ushort)message.Sites.Count));
        WriteUInt16(p[54..], checked((ushort)message.Links.Count));
        WriteUInt16(p[56..], checked((ushort)message.ServiceAreas.Count));
        var offset = RadioFixedLength;
        foreach (var item in message.Sites) { WriteSite(p.Slice(offset, SiteLength), item); offset += SiteLength; }
        foreach (var item in message.Links) { WriteLink(p.Slice(offset, LinkLength), item); offset += LinkLength; }
        foreach (var item in message.ServiceAreas) { WriteServiceArea(p.Slice(offset, ServiceAreaLength), item); offset += ServiceAreaLength; }
        return frame;
    }
    ...
    public static byte[] Serialize(SpectrumSnapshotMessage message, ProtocolVersion version)
    {
        ...
        var p = frame.AsSpan(ProtocolFrameHeader.Size);
        WriteUInt64(p, message.TickCount);
        WriteUInt16(p[8..], checked((ushort)message.Bands.Count));
        WriteUInt16(p[10..], checked((ushort)message.FrequencyBlocks.Count));
        WriteUInt16(p[12..], checked((ushort)message.Conflicts.Count));
        var offset = SpectrumFixedLength;
        foreach (var band in message.Bands) offset += WriteBand(p[offset..], band);
        foreach (var block in message.FrequencyBlocks) { WriteFrequencyBlock(p.Slice(offset, FrequencyBlockLength), block); offset += FrequencyBlockLength; }
        foreach (var conflict in message.Conflicts) offset += WriteConflict(p[offset..], conflict);
        return frame;
    }
    ...
    public static bool TryDeserialize(ReadOnlySpan<byte> frame, out ProtocolEnvelope? envelope, out ProtocolDecodeError error)
    {
        ...
        if (!header.Version.SupportsRadio) { error = ProtocolDecodeError.InvalidPayload; return false; }
    ...
    private static bool TryDeserializeRadio(ProtocolFrameHeader header, ReadOnlySpan<byte> p, out ProtocolEnvelope? envelope, out ProtocolDecodeError error)
    {
        envelope = null;
        if (p.Length < RadioFixedLength) return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload);
        var siteCount = ReadUInt16(p[52..]); var linkCount = ReadUInt16(p[54..]); var areaCount = ReadUInt16(p[56..]);
        int expected;
        try { expected = checked(RadioFixedLength + siteCount * SiteLength + linkCount * LinkLength + areaCount * ServiceAreaLength); }
        catch (OverflowException) { return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload); }
        if (p.Length != expected) return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload);
        var offset = RadioFixedLength;
        var sites = new ProtocolRadioSite[siteCount]; for (var i = 0; i < sites.Length; i++) { sites[i] = ReadSite(p.Slice(offset, SiteLength)); offset += SiteLength; }
        var links = new ProtocolRadioLink[linkCount]; for (var i = 0; i < links.Length; i++) { links[i] = ReadLink(p.Slice(offset, LinkLength)); offset += LinkLength; }
        var areas = new ProtocolRadioServiceArea[areaCount]; for (var i = 0; i < areas.Length; i++) { areas[i] = ReadServiceArea(p.Slice(offset, ServiceAreaLength)); offset += ServiceAreaLength; }
        var message = new RadioSnapshotMessage(ReadRadioStatistics(p), Array.AsReadOnly(sites), Array.AsReadOnly(links), Array.AsReadOnly(areas));
    ...
    private static bool TryDeserializeSpectrum(ProtocolFrameHeader header, ReadOnlySpan<byte> p, out ProtocolEnvelope? envelope, out ProtocolDecodeError error)
    {
        envelope = null;
        if (p.Length < SpectrumFixedLength) return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload);
        var bandCount = ReadUInt16(p[8..]); var blockCount = ReadUInt16(p[10..]); var conflictCount = ReadUInt16(p[12..]);
        var offset = SpectrumFixedLength;
        var bands = new ProtocolSpectrumBand[bandCount];
        for (var i = 0; i < bands.Length; i++) if (!TryReadBand(p, ref offset, out bands[i])) return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload);
        var blocks = new ProtocolFrequencyBlock[blockCount];
        for (var i = 0; i < blocks.Length; i++)
        {
            if (offset > p.Length - FrequencyBlockLength) return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload);
            blocks[i] = ReadFrequencyBlock(p.Slice(offset, FrequencyBlockLength)); offset += FrequencyBlockLength;
        }
        var conflicts = new ProtocolSpectrumConflict[conflictCount];
        for (var i = 0; i < conflicts.Length; i++) if (!TryReadConflict(p, ref offset, out conflicts[i])) return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload);
        if (offset != p.Length) return Fail(out envelope, out error, ProtocolDecodeError.InvalidPayload);
        var message = new SpectrumSnapshotMessage(ReadUInt64(p), Array.AsReadOnly(bands), Array.AsReadOnly(blocks), Array.AsReadOnly(conflicts));
    ...
    {
        ArgumentNullException.ThrowIfNull(message.Sites); ArgumentNullException.ThrowIfNull(message.Links); ArgumentNullException.ThrowIfNull(message.ServiceAreas);
        if (message.Sites.Count > ushort.MaxValue || message.Links.Count > ushort.MaxValue || message.ServiceAreas.Count > ushort.MaxValue) throw new ArgumentOutOfRangeException(nameof(message));
    ...
    {
        ArgumentNullException.ThrowIfNull(message.Bands); ArgumentNullException.ThrowIfNull(message.FrequencyBlocks); ArgumentNullException.ThrowIfNull(message.Conflicts);
    ...
    private static bool IsValidRadio(RadioSnapshotMessage message)
    {
        ...
        foreach (var item in message.Sites) if (item.SiteId == 0 || !Enum.IsDefined(item.Kind) || !Finite(item.X) || !Finite(item.Y) || !Finite(item.Z) || !Finite(item.AntennaGainDb) || !NonNegative(item.AntennaHeightMeters)) return false;
        foreach (var item in message.Links) if (item.LinkId == 0 || item.FromSiteId == 0 || item.ToSiteId == 0 || item.FromSiteId == item.ToSiteId || item.FrequencyBlockId == 0 || !NonNegative(item.DistanceMeters) || !Finite(item.PathLossDb) || !Finite(item.ReceivedPowerDbm) || !Finite(item.InterferenceDbm) || !Finite(item.SinrDb) || !NonNegative(item.Utilization) || item.Utilization > 1d + 1e-9 || !Enum.IsDefined(item.State)) return false;
        foreach (var item in message.ServiceAreas) if (item.SiteId == 0 || item.FrequencyBlockId == 0 || !NonNegative(item.RadiusMeters) || !Finite(item.MinimumSinrDb)) return false;
        return true;
    }
    ...
    private static void WriteRadioStatistics(Span<byte> p, ProtocolRadioStatistics v) { WriteUInt32(p,v.SiteCount); WriteUInt32(p[4..],v.BandCount); WriteUInt32(p[8..],v.FrequencyBlockCount); WriteUInt32(p[12..],v.LinkCount); WriteUInt32(p[16..],v.ServiceAreaCount); WriteUInt32(p[20..],v.ConflictCount); WriteUInt32(p[24..],v.HealthyLinkCount); WriteUInt32(p[28..],v.InterferedLinkCount); WriteUInt32(p[32..],v.UnreachableLinkCount); WriteDouble(p[36..],v.PeakSpectrumUtilization); WriteUInt64(p[44..],v.TickCount); }
    private static ProtocolRadioStatistics ReadRadioStatistics(ReadOnlySpan<byte> p) => new(ReadUInt32(p),ReadUInt32(p[4..]),ReadUInt32(p[8..]),ReadUInt32(p[12..]),ReadUInt32(p[16..]),ReadUInt32(p[20..]),ReadUInt32(p[24..]),ReadUInt32(p[28..]),ReadUInt32(p[32..]),ReadDouble(p[36..]),ReadUInt64(p[44..]));
    ...
    private static bool TryReadConflict(ReadOnlySpan<byte> p, ref int offset, out ProtocolSpectrumConflict value) { value=default; if(offset>p.Length-ConflictFixedLength)return false; var first=ReadUInt64(p[offset..]); var second=ReadUInt64(p[(offset+8)..]); var firstSite=ReadUInt64(p[(offset+16)..]); var secondSite=ReadUInt64(p[(offset+24)..]); var length=ReadUInt16(p[(offset+32)..]); if(offset>p.Length-ConflictFixedLength-length)return false; string reason; try{reason=Utf8.GetString(p.Slice(offset+34,length));}catch(DecoderFallbackException){return false;} value=new(first,second,firstSite,secondSite,reason); offset+=34+length; return true; }
    ...
    private static byte Bool(bool v)=>v?(byte)1:(byte)0; private static bool Finite(double v)=>double.IsFinite(v); private static bool Positive(double v)=>double.IsFinite(v)&&v>0d; private static bool NonNegative(double v)=>double.IsFinite(v)&&v>=0d;
