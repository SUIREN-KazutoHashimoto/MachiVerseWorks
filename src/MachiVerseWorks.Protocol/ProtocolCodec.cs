...
public static class ProtocolCodec
{
    ...
    public static byte[] Serialize(IProtocolMessage message, ProtocolVersion? version = null)
    {
        ...
        if (message is RoadNetworkSnapshotMessage road)
        {
            if (!selectedVersion.SupportsRoadNetwork)
            ...
            RoadNetworkProtocolValidator.ValidateOrThrow(road, nameof(message));
        }
    ...
    public static bool TryDeserialize(ReadOnlySpan<byte> frame, out ProtocolEnvelope? envelope, out ProtocolDecodeError error)
    {
        ...
        if (header.MessageType == MessageType.RoadNetworkSnapshot && !header.Version.SupportsRoadNetwork)
        {
        ...
        if (message is RoadNetworkSnapshotMessage road && !RoadNetworkProtocolValidator.IsValid(road))
        {
            error = ProtocolDecodeError.InvalidPayload;
            return false;
        }
    ...
    public static int GetPayloadLength(IProtocolMessage message)
    {
        ArgumentNullException.ThrowIfNull(message);
    ...
    private static int GetRoadPayloadLength(RoadNetworkSnapshotMessage message)
    {
        ...
        return checked(
            RoadHeaderLength
            + message.Nodes.Count * RoadNodeLength
            + message.Segments.Count * RoadSegmentLength
            + message.Lanes.Count * LaneLength
            + message.Connections.Count * LaneConnectionLength
            + message.AccessPoints.Count * RoadAccessPointLength);
    }
    ...
    private static void WritePayload(Span<byte> payload, IProtocolMessage message)
    {
        switch (message)
        {
            ...
            case SubscribeVolumeMessage subscribe:
                if (!IsFiniteVolume(subscribe.MinX, subscribe.MinY, subscribe.MinZ, subscribe.MaxX, subscribe.MaxY, subscribe.MaxZ))
                    throw new ArgumentOutOfRangeException(nameof(message), "Subscribe volume coordinates must be finite and ordered.");
                WriteDouble(payload, subscribe.MinX);
                WriteDouble(payload[8..], subscribe.MinY);
                WriteDouble(payload[16..], subscribe.MinZ);
                WriteDouble(payload[24..], subscribe.MaxX);
                WriteDouble(payload[32..], subscribe.MaxY);
                WriteDouble(payload[40..], subscribe.MaxZ);
                return;
    ...
    private static void WritePedestrian(Span<byte> payload, ulong id, ulong tripRequestId, double x, double y, double z, double velocityX, double velocityY, double velocityZ, double walkingSpeed, ProtocolPedestrianMovementState state, ulong tick)
    {
        ...
        ValidateStableId(tripRequestId, nameof(tripRequestId));
        ValidateFinite(x, nameof(x));
        ValidateFinite(y, nameof(y));
        ValidateFinite(z, nameof(z));
        ValidateFinite(velocityX, nameof(velocityX));
        ...
        WriteUInt64(payload, id);
        WriteUInt64(payload[8..], tripRequestId);
        WriteDouble(payload[16..], x);
        WriteDouble(payload[24..], y);
        WriteDouble(payload[32..], z);
        WriteDouble(payload[40..], velocityX);
        WriteDouble(payload[48..], velocityY);
        WriteDouble(payload[56..], velocityZ);
    ...
    private static void WriteRoad(Span<byte> payload, RoadNetworkSnapshotMessage message)
    {
        ...
        WriteUInt32(payload[8..], checked((uint)message.Nodes.Count));
        WriteUInt32(payload[12..], checked((uint)message.Segments.Count));
        WriteUInt32(payload[16..], checked((uint)message.Lanes.Count));
        WriteUInt32(payload[20..], checked((uint)message.Connections.Count));
        WriteUInt32(payload[24..], checked((uint)message.AccessPoints.Count));
        var offset = RoadHeaderLength;

        foreach (var node in message.Nodes)
        {
            ValidateStableId(node.Id, nameof(message));
            ValidateFinite(node.X, nameof(message));
            ValidateFinite(node.Y, nameof(message));
            ValidateFinite(node.Z, nameof(message));
            ValidateEnum(node.Kind, nameof(message));
            WriteUInt64(payload[offset..], node.Id);
            payload[offset + 8] = (byte)node.Kind;
            WriteDouble(payload[(offset + 9)..], node.X);
            WriteDouble(payload[(offset + 17)..], node.Y);
            WriteDouble(payload[(offset + 25)..], node.Z);
            offset += RoadNodeLength;
        }

        foreach (var segment in message.Segments)
        {
            ValidateStableId(segment.Id, nameof(message));
            ValidateStableId(segment.StartNodeId, nameof(message));
            ValidateStableId(segment.EndNodeId, nameof(message));
            if (segment.StartNodeId == segment.EndNodeId) throw new ArgumentException("Road segments must connect distinct nodes.", nameof(message));
            ValidateEnum(segment.Kind, nameof(message));
            WriteUInt64(payload[offset..], segment.Id);
            payload[offset + 8] = (byte)segment.Kind;
            WriteUInt64(payload[(offset + 9)..], segment.StartNodeId);
            WriteUInt64(payload[(offset + 17)..], segment.EndNodeId);
            offset += RoadSegmentLength;
        }

        foreach (var lane in message.Lanes)
        {
            ValidateStableId(lane.Id, nameof(message));
            ValidateStableId(lane.SegmentId, nameof(message));
            ValidateEnum(lane.Direction, nameof(message));
            if (!double.IsFinite(lane.WidthMeters) || lane.WidthMeters <= 0d || !double.IsFinite(lane.SpeedLimitMetersPerSecond) || lane.SpeedLimitMetersPerSecond <= 0d)
                throw new ArgumentOutOfRangeException(nameof(message));
            WriteUInt64(payload[offset..], lane.Id);
            WriteUInt64(payload[(offset + 8)..], lane.SegmentId);
            payload[offset + 16] = (byte)lane.Direction;
            WriteUInt16(payload[(offset + 17)..], lane.Order);
            WriteDouble(payload[(offset + 19)..], lane.WidthMeters);
            WriteDouble(payload[(offset + 27)..], lane.SpeedLimitMetersPerSecond);
            offset += LaneLength;
        }

        foreach (var connection in message.Connections)
        {
            ValidateStableId(connection.Id, nameof(message));
            ValidateStableId(connection.FromLaneId, nameof(message));
            ValidateStableId(connection.ToLaneId, nameof(message));
            ValidateStableId(connection.ViaNodeId, nameof(message));
            if (connection.FromLaneId == connection.ToLaneId) throw new ArgumentException("Lane connections must connect distinct lanes.", nameof(message));
            ValidateEnum(connection.Movement, nameof(message));
            WriteUInt64(payload[offset..], connection.Id);
            WriteUInt64(payload[(offset + 8)..], connection.FromLaneId);
            WriteUInt64(payload[(offset + 16)..], connection.ToLaneId);
            WriteUInt64(payload[(offset + 24)..], connection.ViaNodeId);
            payload[offset + 32] = (byte)connection.Movement;
            offset += LaneConnectionLength;
        }

        foreach (var access in message.AccessPoints)
        {
            ValidateStableId(access.Id, nameof(message));
            ValidateStableId(access.SegmentId, nameof(message));
            if (!double.IsFinite(access.SegmentOffset)
                || access.SegmentOffset < 0d
                || access.SegmentOffset > 1d
                || (access.BuildingId == 0 && access.PoiId == 0)
                || access.Mode == ProtocolRoadAccessMode.None
                || (access.Mode & ~(ProtocolRoadAccessMode.Motor | ProtocolRoadAccessMode.Foot)) != 0)
            {
                throw new ArgumentOutOfRangeException(nameof(message));
            }
            WriteUInt64(payload[offset..], access.Id);
            WriteUInt64(payload[(offset + 8)..], access.SegmentId);
            WriteDouble(payload[(offset + 16)..], access.SegmentOffset);
            WriteUInt64(payload[(offset + 24)..], access.BuildingId);
            WriteUInt64(payload[(offset + 32)..], access.PoiId);
            payload[offset + 40] = (byte)access.Mode;
    ...
    private static int WriteUtf8String(Span<byte> destination, string value)
    {
        ...
        WriteUInt16(destination, checked((ushort)count));
    ...
    private static bool TryReadAgent(ReadOnlySpan<byte> payload, Func<ulong, double, double, double, double, double, double, ulong, IProtocolMessage> factory, out IProtocolMessage message, out ProtocolDecodeError error)
    {
        ...
        var tick = ReadUInt64(payload[56..]);
        ...
        message = factory(id, x, y, z, velocityX, velocityY, velocityZ, tick);
        error = ProtocolDecodeError.None;
        return true;
    }
    ...
    private static bool TryReadPedestrian(ReadOnlySpan<byte> payload, bool spawn, out IProtocolMessage message, out ProtocolDecodeError error)
    {
        ...
        var id = ReadUInt64(payload);
        var tripRequestId = ReadUInt64(payload[8..]);
        var x = ReadDouble(payload[16..]);
        var y = ReadDouble(payload[24..]);
        var z = ReadDouble(payload[32..]);
        var velocityX = ReadDouble(payload[40..]);
        var velocityY = ReadDouble(payload[48..]);
        var velocityZ = ReadDouble(payload[56..]);
        var walkingSpeed = ReadDouble(payload[64..]);
        var state = (ProtocolPedestrianMovementState)payload[72];
        var tick = ReadUInt64(payload[73..]);
        if (id == 0
            || tripRequestId == 0
            || !double.IsFinite(x)
            || !double.IsFinite(y)
    ...
    private static bool TryReadRoad(ReadOnlySpan<byte> payload, out IProtocolMessage message, out ProtocolDecodeError error)
    {
        ...
        var tick = ReadUInt64(payload);
        var nodeCount = ReadUInt32(payload[8..]);
        var segmentCount = ReadUInt32(payload[12..]);
        var laneCount = ReadUInt32(payload[16..]);
        var connectionCount = ReadUInt32(payload[20..]);
        var accessCount = ReadUInt32(payload[24..]);
        long expected = RoadHeaderLength
            + (long)nodeCount * RoadNodeLength
            + (long)segmentCount * RoadSegmentLength
            + (long)laneCount * LaneLength
            + (long)connectionCount * LaneConnectionLength
            + (long)accessCount * RoadAccessPointLength;
        if (expected != payload.Length || nodeCount > int.MaxValue || segmentCount > int.MaxValue || laneCount > int.MaxValue || connectionCount > int.MaxValue || accessCount > int.MaxValue)
            return InvalidPayload(out message, out error);

        var nodes = new ProtocolRoadNode[(int)nodeCount];
        var segments = new ProtocolRoadSegment[(int)segmentCount];
        var lanes = new ProtocolLane[(int)laneCount];
        var connections = new ProtocolLaneConnection[(int)connectionCount];
        var accessPoints = new ProtocolRoadAccessPoint[(int)accessCount];
        var offset = RoadHeaderLength;

        for (var index = 0; index < nodes.Length; index++)
        {
            var id = ReadUInt64(payload[offset..]);
            var kind = (ProtocolRoadNodeKind)payload[offset + 8];
            var x = ReadDouble(payload[(offset + 9)..]);
            var y = ReadDouble(payload[(offset + 17)..]);
            var z = ReadDouble(payload[(offset + 25)..]);
            if (id == 0 || !Enum.IsDefined(kind) || !double.IsFinite(x) || !double.IsFinite(y) || !double.IsFinite(z)) return InvalidPayload(out message, out error);
            nodes[index] = new(id, kind, x, y, z);
            offset += RoadNodeLength;
        }
        for (var index = 0; index < segments.Length; index++)
        {
            var id = ReadUInt64(payload[offset..]);
            var kind = (ProtocolRoadKind)payload[offset + 8];
            var start = ReadUInt64(payload[(offset + 9)..]);
            var end = ReadUInt64(payload[(offset + 17)..]);
            if (id == 0 || start == 0 || end == 0 || start == end || !Enum.IsDefined(kind)) return InvalidPayload(out message, out error);
            segments[index] = new(id, kind, start, end);
            offset += RoadSegmentLength;
        }
        for (var index = 0; index < lanes.Length; index++)
        {
            var id = ReadUInt64(payload[offset..]);
            var segment = ReadUInt64(payload[(offset + 8)..]);
            var direction = (ProtocolLaneDirection)payload[offset + 16];
            var order = ReadUInt16(payload[(offset + 17)..]);
            var width = ReadDouble(payload[(offset + 19)..]);
            var speed = ReadDouble(payload[(offset + 27)..]);
            if (id == 0 || segment == 0 || !Enum.IsDefined(direction) || !double.IsFinite(width) || width <= 0d || !double.IsFinite(speed) || speed <= 0d) return InvalidPayload(out message, out error);
            lanes[index] = new(id, segment, direction, order, width, speed);
            offset += LaneLength;
        }
        for (var index = 0; index < connections.Length; index++)
        {
            var id = ReadUInt64(payload[offset..]);
            var from = ReadUInt64(payload[(offset + 8)..]);
            var to = ReadUInt64(payload[(offset + 16)..]);
            var via = ReadUInt64(payload[(offset + 24)..]);
            var movement = (ProtocolTurnMovement)payload[offset + 32];
            if (id == 0 || from == 0 || to == 0 || via == 0 || from == to || !Enum.IsDefined(movement)) return InvalidPayload(out message, out error);
            connections[index] = new(id, from, to, via, movement);
            offset += LaneConnectionLength;
        }
        for (var index = 0; index < accessPoints.Length; index++)
        {
            var id = ReadUInt64(payload[offset..]);
            var segment = ReadUInt64(payload[(offset + 8)..]);
            var segmentOffset = ReadDouble(payload[(offset + 16)..]);
            var building = ReadUInt64(payload[(offset + 24)..]);
            var poi = ReadUInt64(payload[(offset + 32)..]);
            var mode = (ProtocolRoadAccessMode)payload[offset + 40];
            if (id == 0
                || segment == 0
                || !double.IsFinite(segmentOffset)
                || segmentOffset < 0d
                || segmentOffset > 1d
                || (building == 0 && poi == 0)
                || mode == ProtocolRoadAccessMode.None
                || (mode & ~(ProtocolRoadAccessMode.Motor | ProtocolRoadAccessMode.Foot)) != 0)
            {
                return InvalidPayload(out message, out error);
            }
            accessPoints[index] = new(id, segment, segmentOffset, building, poi, mode);
            offset += RoadAccessPointLength;
        }

        message = new RoadNetworkSnapshotMessage(tick, nodes, segments, lanes, connections, accessPoints);
        error = ProtocolDecodeError.None;
        return true;
    }
    ...
    private static bool TryReadError(ReadOnlySpan<byte> payload, out IProtocolMessage message, out ProtocolDecodeError error)
    {
        ...
        var count = ReadUInt16(payload[2..]);
        if (count > MaximumErrorParameters) return InvalidPayload(out message, out error);
        var parameters = new List<ProtocolErrorParameter>(count);
        var offset = 4;
        try
        {
    ...
    private static bool TryReadUtf8String(ReadOnlySpan<byte> payload, ref int offset, int maximum, out string value)
    {
        ...
        var length = ReadUInt16(payload[offset..]);
        offset += 2;
        if (length > maximum || offset > payload.Length - length) return false;
        value = Utf8.GetString(payload.Slice(offset, length));
        offset += length;
        return true;
    }
    ...
    {
        if (!double.IsFinite(value)) throw new ArgumentOutOfRangeException(name, value, "Protocol coordinates and velocities must be finite.");
    }

    private static void ValidateStableId(ulong value, string name)
    {
        if (value == 0) throw new ArgumentOutOfRangeException(name, "Protocol stable IDs must be greater than zero.");
    }

    private static void ValidateEnum<T>(T value, string name) where T : struct, Enum
    {
        if (!Enum.IsDefined(value)) throw new ArgumentOutOfRangeException(name, value, $"{typeof(T).Name} is invalid.");
    }
