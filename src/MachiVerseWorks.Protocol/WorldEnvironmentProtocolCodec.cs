...
public static class WorldEnvironmentProtocolCodec
{
    ...
    private const byte MaximumHemisphere = 1;
    private const byte MaximumLandform = 2;
    private const byte MaximumSurfaceWater = 5;
    private const byte MaximumTerrainMaterial = 5;
    private const byte MaximumFeatureType = 15;
    private const byte MaximumToponymProvenanceKind = 1;
    ...
    private static bool IsValid(WorldEnvironmentSnapshotMessage message)
    {
        ...
        if (message.Config.WorldSeed == 0 || message.Config.Hemisphere > MaximumHemisphere || !Finite(message.Config.GeographicNorthX) || !Finite(message.Config.GeographicNorthY) || !Finite(message.Config.LatitudeDegrees) || message.Config.LatitudeDegrees is < -90d or > 90d) return false;
