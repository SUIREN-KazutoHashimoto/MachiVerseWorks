...
internal sealed class ServerOptions
{
    ...
    public static ServerOptions Load(IConfiguration configuration)
    {
        ...
        {
            throw new InvalidOperationException("Server:Port must be between 0 and 65535.");
        }
        ...
        {
            throw new InvalidOperationException($"Simulation:TickRate must be between 1 and {ushort.MaxValue}.");
        }
        ...
        {
            throw new InvalidOperationException("Simulation:SpatialCellSize must be finite and greater than zero.");
        }

        var initialAgentCount = ReadInt32(configuration, "Simulation:InitialAgentCount", 1000);
        if (initialAgentCount < 0)
        {
            throw new InvalidOperationException("Simulation:InitialAgentCount cannot be negative.");
        }
    ...
    private static int ReadInt32(IConfiguration configuration, string key, int defaultValue)
    {
        ...
        {
            throw new InvalidOperationException($"{key} must be a 32-bit integer.");
        }
    ...
    private static ulong ReadUInt64(IConfiguration configuration, string key, ulong defaultValue)
    {
        ...
        {
            throw new InvalidOperationException($"{key} must be an unsigned 64-bit integer.");
        }
    ...
    private static double ReadDouble(IConfiguration configuration, string key, double defaultValue)
    {
        ...
        {
            throw new InvalidOperationException($"{key} must be a floating-point number.");
        }
