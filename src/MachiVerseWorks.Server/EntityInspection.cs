...
internal static class EntityInspectionMessageMapper
{
    ...
    public static EntityInspectionSnapshotMessage Create(
        ...
        IReadOnlyDictionary<ulong, TrainSnapshot> trains,
        PersistentRegionalEvolutionSnapshotMessage? regional) =>
        Create(target, population, vehicles, trains, new Dictionary<ulong, ulong>(), regional);
        ...
        PersistentRegionalEvolutionSnapshotMessage? regional)
    {
        ...
        ArgumentNullException.ThrowIfNull(generatedBuildingIds);

        return target.EntityType switch
        {
            ProtocolEntityType.Person => CreatePerson(target.EntityId, population, generatedBuildingIds, regional?.CurrentYear),
    ...
    private static EntityInspectionSnapshotMessage CreatePerson(
        ulong id,
        PopulationPublishSnapshot population,
        IReadOnlyDictionary<ulong, ulong> generatedBuildingIds,
        int? currentYear)
    {
        ...
        AddBuildingRelation(relations, generatedBuildingIds, "residence", debug.ResidenceBuildingId);
        AddBuildingRelation(relations, generatedBuildingIds, "currentLocation", debug.CurrentBuildingId);
        AddBuildingRelation(relations, generatedBuildingIds, "destination", debug.DestinationBuildingId);
        ...
        PersistentRegionalEvolutionSnapshotMessage? regional)
    {
        ...
        var relations = regional.Relations
            ...
            .Select(item => new ProtocolInspectionRelation(
                ...
                item.Strength))
            .ToArray();
        var recent = RecentEvents(regional.Events.Where(item => item.SettlementId == id), regional.CurrentYear);
        ...
        PersistentRegionalEvolutionSnapshotMessage? regional)
    {
        ...
        var recent = parcel.BuildingId == 0
            ? []
            : RecentEvents(regional!.Events.Where(item => item.BuildingId == parcel.BuildingId), regional.CurrentYear);
        ...
        PersistentRegionalEvolutionSnapshotMessage? regional)
    {
        ...
        var recent = RecentEvents(regional.Events.Where(item => item.BuildingId == id), regional.CurrentYear);
        return Snapshot(ProtocolEntityType.Building, id, regional.TickCount, regional.CurrentYear, fields, relations, recent);
    }

    private static ProtocolInspectionEvent[] RecentEvents(
        IEnumerable<ProtocolRegionalEvolutionEvent> events,
        int currentYear) =>
        events.Where(item => item.Year <= currentYear
                             && (long)currentYear - item.Year < EntityInspectionProtocolCodec.MaximumRecentYears)
            .OrderByDescending(item => item.Year)
            .ThenByDescending(item => item.EventId)
            .Take(EntityInspectionProtocolCodec.MaximumRecentEvents)
            .Select(item => new ProtocolInspectionEvent(
                item.EventId,
                item.Year,
                ((RegionalEvolutionEventKind)item.Kind).ToString(),
                item.Reason))
            .ToArray();
    ...
    private static void AddBuildingRelation(
        List<ProtocolInspectionRelation> relations,
        IReadOnlyDictionary<ulong, ulong> generatedBuildingIds,
        string kind,
        ulong materializedBuildingId)
    {
        if (materializedBuildingId == 0
            || !generatedBuildingIds.TryGetValue(materializedBuildingId, out var generatedBuildingId))
        {
            return;
        }

        relations.Add(new ProtocolInspectionRelation(
            kind,
            ProtocolEntityType.Building,
            generatedBuildingId,
            1d));
    }
