...
internal sealed class GasFixtureHostedService(SimulationRuntime simulation, IConfiguration configuration) : BackgroundService
{
    ...
    private void Seed(SimulationWorld world)
    {
        ...
        var consumerBuilding = world.CreateBuilding(new WorldVolume(40d, 20d, 0d, 50d, 30d, 8d), BuildingKind.Commercial);
        var sourceNode = world.CreateGasNode(new WorldPoint(5d, 15d, 0d), GasNodeKind.Source);
        var serviceNode = world.CreateGasNode(new WorldPoint(45d, 25d, 0d), GasNodeKind.Service);
        ...
        var roadEnd = world.CreateRoadNode(new WorldPoint(45d, 35d, 0d));
        ...
        var consumerAccess = world.CreateRoadAccessPoint(segment, 0.95d, consumerBuilding, mode: RoadAccessMode.Motor);
