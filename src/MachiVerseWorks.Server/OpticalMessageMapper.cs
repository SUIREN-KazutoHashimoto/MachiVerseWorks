...
internal static class OpticalMessageMapper
{
    ...
    public static OpticalSnapshotMessage Create(OpticalSnapshot snapshot)
    {
        ...
        var demands = snapshot.Demands.Where(item => nodeIds.Contains(item.NodeId.Value)).Take(MaximumDebugEntries)
            .Select(static item => new ProtocolOpticalDemand(item.Id.Value, item.NodeId.Value, (ProtocolOpticalDemandKind)item.Kind, item.BuildingId?.Value ?? 0, item.EstablishmentId?.Value ?? 0, item.BaseDemandGigabitsPerSecond, item.DemandGigabitsPerSecond, item.AllocatedGigabitsPerSecond, (ProtocolOpticalQualityState)item.QualityState, item.BackhaulId?.Value ?? 0)).ToArray();
