...
const timeout = window.setTimeout(() => fail(new Error('Timed out waiting for Radio/Spectrum entities, interference, outage, recovery, and debug rendering.')), 60_000);
...
const outageEmissionIds = new Set();
  ...
  try {
    ...
    if (message.type === SPECTRUM_SNAPSHOT_MESSAGE_TYPE) {
      ...
      for (const emission of message.emissions) {
        if (!emission.isOperational) {
          outageEmissionIds.add(emission.emissionId.toString());
          sawOutage = true;
        } else if (outageEmissionIds.has(emission.emissionId.toString())) {
          sawRecovery = true;
        }
      }
    ...
    if (!sawExplicitEntities || !sawMultipleFrequencies || !sawInterference || !sawConflict || !sawOutage || !sawRecovery || !sawDebugRender || latestRadio === null || latestSpectrum === null) return;
    clearTimeout(timeout);
    socket.close(1000, 'done');
    overlay.dispose();
    result.dataset.status = 'passed';
    result.textContent = `Phase 28 E2E passed: tick=${latestRadio.statistics.tickCount.toString()}, sites=${latestRadio.sites.length}, emissions=${latestRadio.emissions.length}, links=${latestRadio.links.length}, conflicts=${latestSpectrum.conflicts.length}`;
  } catch (error) {
    fail(error);
  }
});
