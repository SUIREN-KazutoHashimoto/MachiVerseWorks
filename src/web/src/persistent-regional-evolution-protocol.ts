...
export interface ParcelEvolutionObservation {
  ...
  readonly developmentState: number;
...
export interface ServiceCatchmentObservation {
  readonly settlementId: bigint;
  readonly kind: RegionalServiceKind;
  readonly radiusMeters: number;
  readonly coverage: number;
}

export interface InfrastructureDemandObservation {
  readonly settlementId: bigint;
  readonly kind: InfrastructureDemandKind;
  readonly demand: number;
  readonly reason: string;
}

export interface RegionalRelationObservation {
  readonly relationId: bigint;
  readonly fromSettlementId: bigint;
  readonly toSettlementId: bigint;
  readonly kind: RegionalRelationKind;
  readonly strength: number;
  readonly isActive: boolean;
  readonly sinceYear: number;
}

export interface RegionalEvolutionEventObservation {
  readonly eventId: bigint;
  readonly year: number;
  readonly kind: RegionalEvolutionEventKind;
  readonly settlementId: bigint;
  readonly buildingId: bigint;
  readonly reason: string;
}

export interface RegionalCommutingFlowObservation {
  readonly fromSettlementId: bigint;
  readonly toSettlementId: bigint;
  readonly workerCount: number;
}

export interface RegionalFreightFlowObservation {
  readonly fromSettlementId: bigint;
  readonly toSettlementId: bigint;
...
export interface PersistentRegionalEvolutionProtocolEnvelope {
  readonly version: ProtocolVersion;
  readonly message: PersistentRegionalEvolutionSnapshotMessage;
}
...
interface WirePersistentRegionalEvolutionSnapshot extends Omit<PersistentRegionalEvolutionSnapshotMessage, 'type' | 'tickCount' | 'settlements' | 'parcels' | 'buildings' | 'serviceCatchments' | 'infrastructureDemands' | 'relations' | 'events' | 'commutingFlows' | 'freightFlows'> {
...
function normalizeAndValidateSnapshot(raw: WirePersistentRegionalEvolutionSnapshot): PersistentRegionalEvolutionSnapshotMessage {
  if (!isRecord(raw) || !Array.isArray(raw.settlements) || !Array.isArray(raw.parcels) || !Array.isArray(raw.buildings)
    ...
    throw new ProtocolDecodeFailure('PersistentRegionalEvolution snapshot shape is invalid.');
  }
  ...
  for (const item of parcels) if (!settlementIds.has(item.settlementId)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Parcel settlement reference is invalid.');
  ...
  for (const item of buildings) if (!parcelIds.has(item.parcelId)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Building parcel reference is invalid.');
  for (const item of parcels) if (item.buildingId !== 0n && !buildingIds.has(item.buildingId)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Parcel building reference is invalid.');

  const serviceCatchments = Object.freeze(raw.serviceCatchments.map(normalizeServiceCatchment));
  for (const item of serviceCatchments) if (!settlementIds.has(item.settlementId)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution ServiceCatchment settlement reference is invalid.');
  const infrastructureDemands = Object.freeze(raw.infrastructureDemands.map(normalizeInfrastructureDemand));
  for (const item of infrastructureDemands) if (!settlementIds.has(item.settlementId)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution InfrastructureDemand settlement reference is invalid.');
  ...
  for (const item of relations) if (!validSettlementPair(item.fromSettlementId, item.toSettlementId, settlementIds)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Relation settlement reference is invalid.');

  let previousEventId = 0n;
  const events = Object.freeze(raw.events.map((item) => {
    const normalized = normalizeEvent(item, raw.currentYear);
    if (normalized.eventId <= previousEventId) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Event IDs must be strictly increasing.');
    previousEventId = normalized.eventId;
    return normalized;
  }));
  for (const item of events) {
    if (!settlementIds.has(item.settlementId)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Event settlement reference is invalid.');
    if (item.buildingId !== 0n && !buildingIds.has(item.buildingId)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Event building reference is invalid.');
  }

  const commutingFlows = Object.freeze(raw.commutingFlows.map(normalizeCommutingFlow));
  for (const item of commutingFlows) if (!validSettlementPair(item.fromSettlementId, item.toSettlementId, settlementIds)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution commuting flow reference is invalid.');
  const freightFlows = Object.freeze(raw.freightFlows.map(normalizeFreightFlow));
  for (const item of freightFlows) if (!validSettlementPair(item.fromSettlementId, item.toSettlementId, settlementIds)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution freight flow reference is invalid.');
...
function normalizeSettlement(raw: WireSettlementEvolution, currentYear: number): SettlementEvolutionObservation {
  if (!isRecord(raw) || !finite(raw.x) || !finite(raw.y) || !finite(raw.z) || !integerAtLeast(raw.population, 0) || !integerAtLeast(raw.jobs, 0)
    ...
    || !enumRange(raw.scale, SettlementScale.Hamlet, SettlementScale.Metropolis) || !enumRange(raw.trend, SettlementTrend.Growing, SettlementTrend.Dormant)
...
function normalizeParcel(raw: WireParcelEvolution): ParcelEvolutionObservation {
  if (!isRecord(raw) || !unit(raw.developmentDemand) || !unit(raw.landValue) || !integerRange(raw.developmentState, 0, 3)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Parcel values are invalid.');
...
function normalizeBuilding(raw: WireBuildingLifecycle, currentYear: number): BuildingLifecycleObservation {
  if (!isRecord(raw) || !integerRange(raw.use, 0, 6) || !Number.isInteger(raw.builtYear) || raw.builtYear > currentYear
    || !Number.isInteger(raw.lastChangedYear) || raw.lastChangedYear > currentYear || !unit(raw.condition) || !unit(raw.occupancy)
    || !integerAtLeast(raw.capacity, 0) || !enumRange(raw.status, BuildingLifecycleStatus.Active, BuildingLifecycleStatus.Demolished)) {
    throw new ProtocolDecodeFailure('PersistentRegionalEvolution Building values are invalid.');
  }
...
function normalizeServiceCatchment(raw: WireServiceCatchment): ServiceCatchmentObservation {
  if (!isRecord(raw) || !enumRange(raw.kind, RegionalServiceKind.Commerce, RegionalServiceKind.Medical) || !positive(raw.radiusMeters) || !unit(raw.coverage)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution ServiceCatchment values are invalid.');
...
function normalizeInfrastructureDemand(raw: WireInfrastructureDemand): InfrastructureDemandObservation {
  if (!isRecord(raw) || !enumRange(raw.kind, InfrastructureDemandKind.Road, InfrastructureDemandKind.Utility) || !unit(raw.demand) || !validReason(raw.reason)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution InfrastructureDemand values are invalid.');
...
function normalizeRelation(raw: WireRegionalRelation, currentYear: number): RegionalRelationObservation {
  if (!isRecord(raw) || !enumRange(raw.kind, RegionalRelationKind.Commuting, RegionalRelationKind.Metro) || !unit(raw.strength) || typeof raw.isActive !== 'boolean'
    || !Number.isInteger(raw.sinceYear) || raw.sinceYear > currentYear) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Relation values are invalid.');
  return Object.freeze({ ...raw, relationId: parsePositiveUInt64(raw.relationId, 'Relation ID'), fromSettlementId: parsePositiveUInt64(raw.fromSettlementId, 'Relation from settlement ID'), toSettlementId: parsePositiveUInt64(raw.toSettlementId, 'Relation to settlement ID') });
}

function normalizeEvent(raw: WireRegionalEvolutionEvent, currentYear: number): RegionalEvolutionEventObservation {
  if (!isRecord(raw) || !Number.isInteger(raw.year) || raw.year > currentYear || !enumRange(raw.kind, RegionalEvolutionEventKind.Growth, RegionalEvolutionEventKind.RegionalRelationEnded) || !validReason(raw.reason)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution Event values are invalid.');
...
function normalizeCommutingFlow(raw: WireRegionalCommutingFlow): RegionalCommutingFlowObservation {
  if (!isRecord(raw) || !integerAtLeast(raw.workerCount, 1)) throw new ProtocolDecodeFailure('PersistentRegionalEvolution commuting flow values are invalid.');
  return Object.freeze({ ...raw, fromSettlementId: parsePositiveUInt64(raw.fromSettlementId, 'Commuting from settlement ID'), toSettlementId: parsePositiveUInt64(raw.toSettlementId, 'Commuting to settlement ID') });
}

function normalizeFreightFlow(raw: WireRegionalFreightFlow): RegionalFreightFlowObservation {
  if (!isRecord(raw) || !nonNegative(raw.quantity) || !integerAtLeast(raw.shipmentCount, 1) || !nonNegative(raw.deliveredQuantity) || raw.deliveredQuantity > raw.quantity) throw new ProtocolDecodeFailure('PersistentRegionalEvolution freight flow values are invalid.');
  return Object.freeze({ ...raw, fromSettlementId: parsePositiveUInt64(raw.fromSettlementId, 'Freight from settlement ID'), toSettlementId: parsePositiveUInt64(raw.toSettlementId, 'Freight to settlement ID'), commodityId: parsePositiveUInt64(raw.commodityId, 'Freight commodity ID') });
}

function validSettlementPair(from: bigint, to: bigint, settlements: ReadonlySet<bigint>): boolean { return from !== to && settlements.has(from) && settlements.has(to); }
...
function enumRange(value: unknown, minimum: number, maximum: number): value is number { return integerRange(value, minimum, maximum); }
