...
export function decodeOpticalFrame(frame:ArrayBuffer):OpticalProtocolEnvelope {
  ...
  let c=o+FIXED_PAYLOAD_LENGTH; const nodes:OpticalNode[]=[]; for(let i=0;i<nc;i++){nodes.push({nodeId:v.getBigUint64(c,true),kind:v.getUint8(c+8) as OpticalNodeKind,x:v.getFloat64(c+9,true),y:v.getFloat64(c+17,true),z:v.getFloat64(c+25,true)});c+=NODE_LENGTH;}
  const fiberCables:FiberCable[]=[]; for(let i=0;i<cc;i++){const a=v.getUint8(c+48),b=v.getUint8(c+49); if(a>1||b>1)throw new ProtocolDecodeFailure('Invalid Optical cable flags.'); fiberCables.push({cableId:v.getBigUint64(c,true),fromNodeId:v.getBigUint64(c+8,true),toNodeId:v.getBigUint64(c+16,true),capacityGigabitsPerSecond:v.getFloat64(c+24,true),loadGigabitsPerSecond:v.getFloat64(c+32,true),utilization:v.getFloat64(c+40,true),isInService:a!==0,isCongested:b!==0});c+=CABLE_LENGTH;}
  const equipment:OpticalEquipment[]=[]; for(let i=0;i<ec;i++){equipment.push({equipmentId:v.getBigUint64(c,true),nodeId:v.getBigUint64(c+8,true),kind:v.getUint8(c+16) as OpticalEquipmentKind,buildingId:v.getBigUint64(c+17,true),establishmentId:v.getBigUint64(c+25,true),capacityGigabitsPerSecond:v.getFloat64(c+33,true),requiresPower:v.getUint8(c+41)!==0,isInService:v.getUint8(c+42)!==0,isPowered:v.getUint8(c+43)!==0,isOperational:v.getUint8(c+44)!==0});c+=EQUIPMENT_LENGTH;}
  const backhauls:OpticalBackhaul[]=[]; for(let i=0;i<bc;i++){backhauls.push({backhaulId:v.getBigUint64(c,true),nodeId:v.getBigUint64(c+8,true),capacityGigabitsPerSecond:v.getFloat64(c+16,true),allocatedGigabitsPerSecond:v.getFloat64(c+24,true),utilization:v.getFloat64(c+32,true),isInService:v.getUint8(c+40)!==0,isOperational:v.getUint8(c+41)!==0});c+=BACKHAUL_LENGTH;}
  const demands:OpticalDemand[]=[]; for(let i=0;i<dc;i++){demands.push({demandId:v.getBigUint64(c,true),nodeId:v.getBigUint64(c+8,true),kind:v.getUint8(c+16) as OpticalDemandKind,buildingId:v.getBigUint64(c+17,true),establishmentId:v.getBigUint64(c+25,true),baseDemandGigabitsPerSecond:v.getFloat64(c+33,true),demandGigabitsPerSecond:v.getFloat64(c+41,true),allocatedGigabitsPerSecond:v.getFloat64(c+49,true),qualityState:v.getUint8(c+57) as OpticalQualityState,backhaulId:v.getBigUint64(c+58,true),estimatedLatencyMilliseconds:v.getFloat64(c+66,true)});c+=DEMAND_LENGTH;}
  if(nodes.some(n=>n.nodeId===0n)||fiberCables.some(x=>x.cableId===0n||x.fromNodeId===x.toNodeId||!positive(x.capacityGigabitsPerSecond)||!nonNegative(x.utilization))||demands.some(x=>x.demandId===0n||!nonNegative(x.demandGigabitsPerSecond)||!nonNegative(x.allocatedGigabitsPerSecond)||!nonNegative(x.estimatedLatencyMilliseconds))) throw new ProtocolDecodeFailure('Optical snapshot contains invalid values.');
  return {version,message:{type:OPTICAL_SNAPSHOT_MESSAGE_TYPE,statistics,nodes,fiberCables,equipment,backhauls,demands}};
}
function positive(x:number):boolean{return Number.isFinite(x)&&x>0;} function nonNegative(x:number):boolean{return Number.isFinite(x)&&x>=0;}
