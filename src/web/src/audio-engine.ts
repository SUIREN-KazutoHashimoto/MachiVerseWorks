...
export class AudioEngine {
  ...
  private readonly categoryVolumes = new Map<AudioCategory, number>([
    ['music', 1],
    ['ui', 1],
    ['ambient', 1],
    ['world', 1],
  ...
  public onStateChanged(callback: (state: AudioEngineState) => void): void {
  ...
  public async preload(cueIds: readonly string[]): Promise<void> {
    if (this.stateValue === 'unavailable') {
      return;
    }
    await Promise.all(cueIds.map((cueId) => this.loadBuffer(cueId)));
  }
  ...
  public async syncSpatialVoices(listener: Point3D): Promise<void> {
    ...
    const candidates: VoiceCandidate[] = [...this.emitters.values()].map((emitter) => ({
      id: emitter.id,
    ...
    for (const id of this.activeEmitterVoices.keys()) {
      if (!selected.has(id)) {
        this.stopEmitterVoice(id);
      }
    }
  ...
  public syncListenerFromCamera(camera: ThreeCameraLike): void {
    if (this.stateValue !== 'running' || this.context === null) {
      return;
    }
    ...
    listener.forwardX.setValueAtTime(direction.x, time);
    listener.forwardY.setValueAtTime(direction.y, time);
    listener.forwardZ.setValueAtTime(direction.z, time);
  ...
  public async setAmbientLayer(
    ...
    if (voice === undefined && desiredGain > 0) {
      voice = await this.startAmbientVoice(key, cueId);
    }
  ...
  private getCategoryGain(category: AudioCategory): GainNode {
    const gain = this.categoryGains.get(category);
    if (gain === undefined) {
      throw new Error(`Audio category ${category} is not initialized.`);
    }
    return gain;
  }
  ...
  private async loadBuffer(cueId: string): Promise<AudioBuffer> {
    ...
    const promise = fetch(`/audio/${cue.source}`)
      .then((response) => {
        if (!response.ok) {
          throw new Error(`Failed to load audio cue ${cueId}.`);
        }
  ...
  private async ensureEmitterVoice(id: string): Promise<void> {
    if (this.activeEmitterVoices.has(id) || this.stateValue !== 'running') {
      return;
    }
    const emitter = this.emitters.get(id);
    if (emitter === undefined) {
    ...
    const cue = getCue(emitter.cueId);
    const buffer = await this.loadBuffer(emitter.cueId);
    if (this.activeEmitterVoices.has(id) || !this.emitters.has(id) || this.stateValue !== 'running') {
      return;
    }
    ...
    const panner = this.createPanner(cue, emitter.position);
    source.connect(gain);
    gain.connect(panner);
    panner.connect(this.getCategoryGain(cue.category));
    source.start();
    this.activeEmitterVoices.set(id, { cueId: emitter.cueId, source, panner });
  }
  ...
  private stopAllEmitterVoices(): void {
    for (const id of [...this.activeEmitterVoices.keys()]) {
      this.stopEmitterVoice(id);
    }
  }

  private async startAmbientVoice(key: string, cueId: string): Promise<AmbientVoice | undefined> {
    const buffer = await this.loadBuffer(cueId);
    if (this.stateValue !== 'running' || this.context === null) {
      return undefined;
    }
    ...
    this.ambientVoices.set(key, voice);
    source.start();
    return voice;
  }
  ...
  private applyMasterGain(): void {
    if (this.masterGain !== null) {
      this.masterGain.gain.value = resolveMasterGain(this.muted, this.masterVolume);
    }
  }
...
function getCue(cueId: string): AudioCueDefinition {
