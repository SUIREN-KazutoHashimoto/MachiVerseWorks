...
internal sealed class PoiStore
{
    ...
    {
        if (nextId == ulong.MaxValue) throw new OverflowException("POI ID capacity has been exhausted.");
        var id = new PoiId(nextId++);
        items.Add(id, new PoiSnapshot(id, kind, position, buildingId));
        return id;
    }

    public bool Update(PoiId id, PoiKind kind, WorldPoint position, BuildingId? buildingId)
    {
        if (!items.ContainsKey(id)) return false;
        items[id] = new PoiSnapshot(id, kind, position, buildingId);
        return true;
    }

    public bool Remove(PoiId id) => items.Remove(id);
    public bool TryGetSnapshot(PoiId id, out PoiSnapshot snapshot) => items.TryGetValue(id, out snapshot);

    public bool ContainsBuildingReference(BuildingId buildingId)
    {
        foreach (var poi in items.Values) if (poi.BuildingId == buildingId) return true;
        return false;
    }
    ...
    public IReadOnlyList<SimulationPoiCheckpoint> CreateCheckpoint()
    {
        ...
        for (var index = 0; index < result.Length; index++)
        {
            ...
            result[index] = new SimulationPoiCheckpoint(snapshot.Id, snapshot.Kind, snapshot.Position, snapshot.BuildingId);
        }
        return result;
    }
    ...
    {
        items.Clear();
        foreach (var poi in pois) items.Add(poi.Id, new PoiSnapshot(poi.Id, poi.Kind, poi.Position, poi.BuildingId));
