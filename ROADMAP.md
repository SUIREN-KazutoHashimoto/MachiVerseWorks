...
このファイルは、MachiVerseWorks の**実際に完了判定できる作業項目**を管理するためのロードマップです。

旧 Machi-Sim では目標単位が大きく、長期間チェックを閉じられない項目が増えたため、MachiVerseWorks ではタスクを小さく分割して管理します。

## 運用ルール

- チェックボックスを付けるのは、単独で完了判定できる作業だけとする。
- 1タスクは原則として「1つの観測可能な成果」を持つ。
- 1タスク内に独立した成果が複数ある場合は、着手前または判明した時点で分割する。
- 「交通を完成させる」「経済を実装する」のような大テーマはチェックボックスにしない。
...
- 優先順位は状況に応じて変更してよい。Task ID は参照用なので、並び替えても変更しない。
- 全項目が完了した大きなセクションは、必要に応じて履歴を `docs/archive/` へ移して ROADMAP を読みやすく保つ。
...
「ほぼ完了」「一部完了」は `[x]` にしない。残作業を別タスクへ明示的に切り出した場合のみ、元タスクを完了としてよい。

---

## 0. リポジトリ初期セットアップ

- [x] `SET-001` 基本ディレクトリ構成を作成する
- [x] `SET-002` ルート `README.md` / `AGENT.md` / `AGENTS.md` を整備する
- [x] `SET-003` `.gitignore` / `.gitattributes` / `.editorconfig` を整備する
- [x] `SET-004` Apache-2.0 の `LICENSE` / `NOTICE` / 第三者通知の基盤を整備する
- [x] `SET-005` CONTRIBUTING / SECURITY / Code of Conduct / Issue・PRテンプレートを整備する
- [x] `SET-006` Legacy から再利用する開発知見と移行方針を整理する
- [x] `SET-007` Server-authoritative 構成の ADR とアーキテクチャ概要を整備する
- [x] `SET-008` 将来の多言語対応を壊さない localization 境界を整備する
- [x] `SET-009` GitHub Actions の CI / CodeQL / Dependency Review / Dependabot を整備する
- [x] `SET-010` `.NET` SDK を `global.json` で固定する
- [x] `SET-011` Coding Guidelines と Performance Guidelines を整備する
- [x] `SET-012` 細粒度タスク管理用の `ROADMAP.md` を導入する
...
以下は**テーマ**であり、完了チェック対象ではありません。着手するときに、その時点の設計に合わせて上記と同程度の粒度へ分解します。
