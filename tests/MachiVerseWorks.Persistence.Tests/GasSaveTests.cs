...
public sealed class GasSaveTests
{
    ...
    public void SavePreservesGasStateAndDeterministicContinuation()
    {
        ...
        StringAssert.Contains(json, "\"gas\"");
    ...
    public void ExistingFormatElevenSaveWithoutGasRestoresEmptyGasState()
    {
        ...
        var json = Encoding.UTF8.GetString(bytes);
        json = json.Replace("      \"gas\": null\n", string.Empty, StringComparison.Ordinal);

        var restored = WorldSaveSerializer.Deserialize(Encoding.UTF8.GetBytes(json));

        Assert.AreEqual(0, restored.GasNodeCount);
        Assert.AreEqual(0, restored.GasPipelineCount);
        Assert.AreEqual(0, restored.GasServicePointCount);
    }
    ...
    private static SimulationWorld CreateWorld()
    {
        ...
        world.CreateGasPipeline(sourceNode, serviceNode, 20d);
        world.CreateGasSource(sourceNode, 20d);
        world.CreateGasStorage(sourceNode, 100d, 30d, 5d);
        world.CreatePipedGasServicePoint(serviceNode, 10d, buildingId: building);
        return world;
    }
}
