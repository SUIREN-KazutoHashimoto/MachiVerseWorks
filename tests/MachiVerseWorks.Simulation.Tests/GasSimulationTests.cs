...
public sealed class GasSimulationTests
{
    ...
    public void PipelineCapacityCreatesConstrainedGasService()
    {
        ...
        Assert.AreEqual(snapshot.DemandCubicMetersPerDay - 5d, snapshot.UnservedCubicMetersPerDay, 1e-9);
        Assert.AreNotEqual(snapshot.BaseDemandCubicMetersPerDay, snapshot.DemandCubicMetersPerDay);
    }
    ...
    public void PipelineOutageIsObservableAndRecoverable()
    {
        ...
        world.SetGasPipelineInService(pipeline, false); world.Step();
        ...
        world.SetGasPipelineInService(pipeline, true); world.Step();
    ...
    public void DeliveredGasUsesLogisticsInventory()
    {
        ...
        var start = world.CreateRoadNode(new WorldPoint(0, 0, 0)); var end = world.CreateRoadNode(new WorldPoint(20, 0, 0));
        var segment = world.CreateRoadSegment(start, end, RoadKind.Local); world.CreateLane(segment, LaneDirection.Forward, 0, speedLimitMetersPerSecond: 12d);
        var access = world.CreateRoadAccessPoint(segment, 0.05, building, mode: RoadAccessMode.Motor);
        var company = world.CreateCompany(IndustrySector.Services, 100_000, 0d); var establishment = world.CreateEstablishment(company, buildingId: building);
    ...
    [TestMethod]
    public void GasNodeQueryUsesThreeDimensionalVolume()
    {
        var world = new SimulationWorld(new SimulationConfig(tickRate: 1, seed: 2503));
        var inside = world.CreateGasNode(new WorldPoint(1, 2, 3));
        world.CreateGasNode(new WorldPoint(1, 2, 30));
        var result = world.QueryGasNodes(new WorldVolume(0, 0, 0, 5, 5, 5));
    ...
    public void CheckpointRestoresGasStateAndStableIds()
    {
        ...
        world.SetGasPipelineInService(pipeline, false); world.Step(); var checkpoint = world.CreateCheckpoint();
    ...
    private static SimulationWorld CreatePipedWorld(double sourceCapacity, double pipelineCapacity, double demand, out GasSourceId source, out GasPipelineId pipeline, out GasServicePointId servicePoint, out CompanyId company)
    {
        ...
        company = world.CreateCompany(IndustrySector.Manufacturing, 100_000, 20d); var establishment = world.CreateEstablishment(company, buildingId: building);
        var job = world.CreateJob(establishment, 1, 0); world.AssignEmployment(person, job);
        var sourceNode = world.CreateGasNode(new WorldPoint(-10, 5, 0), GasNodeKind.Source); var serviceNode = world.CreateGasNode(new WorldPoint(5, 5, 0), GasNodeKind.Service);
        pipeline = world.CreateGasPipeline(sourceNode, serviceNode, pipelineCapacity); source = world.CreateGasSource(sourceNode, sourceCapacity); servicePoint = world.CreatePipedGasServicePoint(serviceNode, demand, building, establishment);
        return world;
    }
}
